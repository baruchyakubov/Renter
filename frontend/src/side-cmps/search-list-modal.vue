<template>
    <section v-if="searchCountryList.length" class="search-list-modal">
        <ul>
            <li @click="setInput(place, idx)" class="search-item" v-for="(place, idx) in searchCountryList"
                :key="place">
                <div class="search-icon">
                    <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="presentation"
                        focusable="false"
                        style="position: relative; left: 50%; top: 50%; transform: translate(-50% , -50%); display: block; height: 22px; width: 22px; fill: currentcolor;">
                        <path
                ...
                <p v-if="idx && idx !== 1">{{ place.name }}</p>
                <p v-if="idx === 1">{{ place.city }} , {{ place.country }}</p>
                <p v-if="idx === 0">{{ place.country }}</p>
            </li>
        </ul>
    </section>
...
export default {
    unmounted() {
        this.$store.commit({ type: 'resetSearchList' })
    },
    computed: {
        searchCountryList() {
    ...
    methods: {
        setInput(place, idx) {
            var input
            if (idx === 0) input = place.country
            else if (idx === 1) input = place.city
            else input = place.name
            this.$emit('setInputBySearch', input)
        }
    }
}
</script>
